...
module EmTools
  module Core
    module Rules
      ...
      # rubocop:disable Metrics/ParameterLists -- ported 1:1 from em-tasks; preserve decision-tree shape.
      class DimensionFilter < Strategy
        DIMENSION_MAX_INCH = 12

        FLEXIBLE_DISPLAY_PRODUCT_TYPES = [
          "NECKLACE", "BRACELET", "ANKLET", "CHARM", "PENDANT", "CURTAIN", "BLANKET",
        ].to_set.freeze

        DIM_KEYS = [:width, :height, :length, :diameter].freeze
        ...
        def skip_display_only_dimension?(product, source)
          return false if ["package", "raw"].include?(source.to_s) || source.nil?
        ...
        def item_length_width_height(product)
          ...
          extract_typed_dims(dims, ["width", "height", "length"], "centimeters")
        end
        ...
        def item_display_dimensions(product)
          ...
          result = extract_typed_dims(dims, ["width", "height", "length"], "millimeters")
          return unless result

          # display 尺寸仅在三边都存在时才用于超长判断
          return result if ["width", "height", "length"].all? { |k| result[k] }
        ...
        def length_width_attributes(product)
          ...
          extract_typed_dims(entry, ["length", "width", "height"], "centimeters")
        end
        ...
        def legacy_amazon_dims(legacy)
          ...
          dims = {}
          ["Width", "Height", "Length", "Weight"].each do |k|
            entry = legacy[k]
