  ...
  module Core
    module Sinks
      ...
      class IndexDumper
        ...
        # Builds a dumper from rake-friendly env vars + an explicit ES URL resolver flag.
        #
        # @param env [Hash, ENV-like]
        # @param prefer_data_cluster [Boolean] when true, +DATA_ELASTICSEARCH_URL+ takes precedence.
        # rubocop:disable Metrics/MethodLength -- env-driven factory; one ENV read per option.
        def self.from_env(env: ENV, prefer_data_cluster: false)
          ...
          url = EmTools::Core::Config.elasticsearch_connection_url(
            explicit: explicit, prefer_data_cluster: prefer_data_cluster
          )
          new(
        ...
        def run!
          ensure_output_dir!
          count = stream_hits!
          EmTools::Core::RakeSupport::Result.new(summary: "Wrote #{count} hits to #{@output_path}")
        end
