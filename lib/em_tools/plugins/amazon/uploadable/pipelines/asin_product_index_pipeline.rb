...
module EmTools
  module Plugins
    module Amazon
      module Uploadable
        module Pipelines # -- single cohesive pipeline; split later if it grows further
          ...
          class AsinProductIndexPipeline
            ...
            # -- explicit pipeline knobs; extract Options if this grows further
            def initialize(
              marketplace:,
              sink_index:,
              filter:,
              ...
              if @product_index.nil? || @product_index.empty?
                @product_index = format(
                  "amz_products_api_%s_v2",
                  @marketplace,
                )
              end
