      ...
      module Uploadable
        module Stream
          ...
          module AsinStreamOptions
            ...
            def self.resolve(cfg, cli_since_days:, cli_time_field: nil, cli_cutoff: nil, cli_label: nil,
              cli_label_field: nil)
              cfg ||= {}
              time_field = "auto"
              cutoff_raw = nil
              ...
              unless ["auto", "timestamp", "created_at", "time"].include?(time_field)
                raise ArgumentError,
            ...
            def self.effective_time_field(time_field, asin_index)
            ...
            def self.normalize_label_field(value)
              lf = value.to_s.strip
              lf.empty? ? "label" : lf
            end

            def self.parse_iso8601_utc(raw)
              s = raw.to_s.strip
