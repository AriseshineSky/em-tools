...
module Em
  module Tools
    ...
    # +inventory_feed+ (used with +prune_obsolete+) defaults to each row's CSV +Source+ column (header +Source+
    # becomes field +source+). If +source+ is blank, falls back to +:feed_id+ from options (e.g. YAML override).
    class InventorySync
      BATCH_SIZE = 2000
      ...
      def initialize(sink:, index:, **opts)
        ...
        @prune_obsolete = !!opts[:prune_obsolete]
      end
