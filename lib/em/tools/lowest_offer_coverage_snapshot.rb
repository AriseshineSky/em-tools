...
module Em
  module Tools
    # Writes documents consumed by monitoring-dashboard (index monitoring_lowest_offer_snapshots by default).
    class LowestOfferCoverageSnapshot
      SNAPSHOT_KIND = 'lowest_offer_coverage'

      class << self
        def index_name
          ENV.fetch('MONITORING_LOWEST_OFFER_SNAPSHOT_INDEX', 'monitoring_lowest_offer_snapshots')
        end
