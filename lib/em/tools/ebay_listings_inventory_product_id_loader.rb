...
module Em
  module Tools
    # Loads distinct eBay listing / product ids from +em_inventory+ (or another ES index) for
    # {EbayListingsCoverageQuery} when +EBAY_LISTINGS_COVERAGE_ID_SOURCE=inventory+.
    #
    # Unlike {LowestOfferInventoryAsinLoader}, ids are **not** validated as ASINs (numeric eBay item ids are expected).
    class EbayListingsInventoryProductIdLoader
      ...
      def warn_max_hits
        warn 'EbayListingsInventoryProductIdLoader: hit EBAY_LISTINGS_COVERAGE_INVENTORY_MAX_HITS limit; ' \
