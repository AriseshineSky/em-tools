...
module Em
  module Tools
    ...
    class LowestOfferSeedFiles
      class << self
        def seed_file_present?(dir, mp)
          mp = mp.to_s.downcase
        ...
        def sync_from_gcs(dir, marketplaces:, creds_path:, bucket:, prefix:, force: false)
          FileUtils.mkdir_p(dir)
          pfx = prefix.to_s.sub(%r{/+\z}, '')
          helper = GcsHelper.new(creds_path, bucket, pfx)
