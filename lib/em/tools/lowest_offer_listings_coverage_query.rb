...
module Em
  module Tools
    ...
    class LowestOfferListingsCoverageQuery
      ...
      SOURCE_PRODUCT_ID_JSON_RE = /"source_product_id"\s*:\s*"([^"\\\s]+)"/.freeze
      ...
      def extract_source_product_ids_from_seed_text(text)
        ...
        text.each_line.with_index(1) do |line, lineno|
          ...
          if fb
            ids << fb.upcase
            warn "LowestOfferListingsCoverageQuery seed line #{lineno}: invalid JSON, recovered source_product_id " \
