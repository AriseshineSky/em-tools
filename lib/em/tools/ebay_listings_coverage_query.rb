...
require_relative 'ebay_listings_inventory_product_id_loader'
...
module Em
  module Tools
    ...
    class EbayListingsCoverageQuery
      ...
      def fetch_row
        ...
        warn "EbayListingsCoverageQuery failed for #{@index_name}: #{e.class} #{e.message}"
        seeds = load_seed_ids
      ...
      def load_seed_ids
        ...
        warn "EbayListingsCoverageQuery seed load failed for #{@marketplace}: #{e.message}"
        []
      end
      ...
      def load_ids_from_inventory
        ...
        warn "EbayListingsCoverageQuery inventory load failed for #{@marketplace}: #{e.message}"
        []
      end
      ...
      def extract_source_product_ids_from_seed_text(text)
        ...
        text.each_line.with_index(1) do |line, lineno|
          ...
          if fb
            ids << fb
            warn "EbayListingsCoverageQuery seed line #{lineno}: invalid JSON, recovered source_product_id " \
                 "(#{e.message.to_s.byteslice(0, 120)})"
          else
            warn "EbayListingsCoverageQuery seed line #{lineno}: invalid JSON in column 2 (#{e.message})"
          end
        rescue StandardError => e
          warn "EbayListingsCoverageQuery seed line #{lineno}: #{e.class} #{e.message}"
        end
        ids.uniq.sort
      end
      ...
      def source_product_id_fallback_from_json(json_str)
        ...
        s = m[1].to_s.strip
        LowestOfferAsinPattern.match?(s) || s.match?(/\A\d+\z/) ? s : nil
      end

      def read_seed_text_from_dir
        path = File.join(@seed_dir, "ebay_#{@marketplace}.txt")
        unless File.file?(path)
          warn "EbayListingsCoverageQuery: no seed file #{path}"
          return nil
        end
      ...
      def persist_missing_ids_file(missing_ids)
        ...
        warn "EbayListingsCoverageQuery: could not write missing ids file: #{e.message}"
        nil
      end
