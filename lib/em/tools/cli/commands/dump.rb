      ...
      module Commands
        class Dump
          def run(argv)
            ...
            client = if url_opt
                       Em::Clients::ElasticsearchClient.new(url: url_opt)
                     elsif options[:use_data_cluster]
                       u = Em::Tools::Config.elasticsearch_connection_url(explicit: nil, prefer_data_cluster: true)
                       Em::Clients::ElasticsearchClient.new(url: u)
                     else
                       Support.require_elasticsearch_url!
                       Em::Clients::ElasticsearchClient.new
                     end
