...
RSpec.describe 'Em::Tools::Config exporter / cluster ES URLs' do
  around do |example|
    Em::Tools::Config.reload!
  ...
  it 'resolves exporter cluster via DATA_ELASTICSEARCH_URL for cluster data' do
    expect(Em::Tools::Config.exporter_elasticsearch_url('lotteon_products')).to eq('http://34.16.105.219:9200')
  end

  it 'returns exporter index from settings' do
    expect(Em::Tools::Config.exporter_index('lotteon_products', 'fallback')).to eq('user1_lotteon_products')
  end

  it 'falls back to default elasticsearch_url when exporter is unknown' do
    expect(Em::Tools::Config.exporter_elasticsearch_url('unknown_exporter')).to eq(
      Em::Tools::Config.elasticsearch_url
    )
  end

  it 'exposes data_elasticsearch_url from ENV' do
    expect(Em::Tools::Config.data_elasticsearch_url).to eq('http://34.16.105.219:9200')
  end
end
