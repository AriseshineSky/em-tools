...
# -- spec
RSpec.describe EmTools::Plugins::AmazonLowestOffer::Pipelines::PublishSnapshot do
  ...
  before do
    allow(EmTools::Plugins::AmazonLowestOffer::Sinks::CoverageSnapshot).to receive(:index_name).and_return('idx')
  end
  ...
  context 'when LOWEST_OFFER_ID_SOURCE=inventory' do
    ...
    it 'runs the query without GCS / seed_dir and persists rows' do
      query = instance_double(EmTools::Plugins::AmazonLowestOffer::Queries::ListingsCoverageQuery)
      rows = [{ marketplace: 'US', seed_asins_loaded: 5 }]
      expect(EmTools::Plugins::AmazonLowestOffer::Queries::ListingsCoverageQuery).to receive(:new)
        .with(hash_including(es_client: client, snapshot_time: snapshot_time))
        .and_return(query)
      expect(query).to receive(:fetch_all).and_return(rows)
      expect(EmTools::Plugins::AmazonLowestOffer::Sinks::CoverageSnapshot).to receive(:persist!)
    ...
    it 'raises EmptyResultError when no ASINs loaded' do
      query = instance_double(EmTools::Plugins::AmazonLowestOffer::Queries::ListingsCoverageQuery)
      allow(EmTools::Plugins::AmazonLowestOffer::Queries::ListingsCoverageQuery).to receive(:new).and_return(query)
  ...
  context 'with seed_dir mode and missing GCS credentials' do
    ...
    it 'raises ConfigurationError before running the query' do
      allow(EmTools::Plugins::AmazonLowestOffer::Sources::SeedFiles).to receive(:seed_file_present?).and_return(false)
  ...
  context 'when CLI marketplaces are provided' do
    it 'forwards them lowercased to the query' do
      query = instance_double(EmTools::Plugins::AmazonLowestOffer::Queries::ListingsCoverageQuery)
      expect(EmTools::Plugins::AmazonLowestOffer::Queries::ListingsCoverageQuery).to receive(:new)
        .with(hash_including(marketplaces: %w[us ca]))
        .and_return(query)
      allow(query).to receive(:fetch_all).and_return([])
      allow(EmTools::Plugins::AmazonLowestOffer::Sinks::CoverageSnapshot).to receive(:persist!)
