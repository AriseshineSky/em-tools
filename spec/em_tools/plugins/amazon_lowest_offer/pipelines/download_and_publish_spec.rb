...
RSpec.describe(EmTools::Plugins::AmazonLowestOffer::Pipelines::DownloadAndPublish) do
  it "calls SeedFiles.sync_from_env! then PublishSnapshot#run! and stitches the summary" do
    seed_calls = []
    allow(EmTools::Plugins::AmazonLowestOffer::Sources::SeedFiles).to(receive(:sync_from_env!)) do |**kw|
      seed_calls << kw
    end
    snapshot_result = EmTools::Core::Cli::Runner::Result.new(summary: "Coverage published us=42 (...)")
    snapshot = instance_double(
      EmTools::Plugins::AmazonLowestOffer::Pipelines::PublishSnapshot,
      run!: snapshot_result,
    )
  ...
  it "defaults target_dir to ./tmp when unset" do
    allow(EmTools::Plugins::AmazonLowestOffer::Sources::SeedFiles).to(receive(:sync_from_env!))
    snapshot = instance_double(
      EmTools::Plugins::AmazonLowestOffer::Pipelines::PublishSnapshot,
    ...
    expect(EmTools::Plugins::AmazonLowestOffer::Sources::SeedFiles).to(have_received(:sync_from_env!)
